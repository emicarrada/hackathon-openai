...
def demo_completa():
    ...
    try:
        ...
        # Intentamos acceder a las métricas si el agente las guarda
        metricas1 = getattr(agente, "metricas_run1", None)
        metricas2 = getattr(agente, "metricas_run2", None)
        print(metricas1, metricas2, "imprime metricas")

        if metricas1 and metricas2:
            comparador = ComparadorRuns(metricas1, metricas2)
            comparador.mostrar_comparacion()
        else:
            print("⚠️  No se encontraron métricas para comparar (verificar agente).")
    except Exception as _e:
        print(f"⚠️  No se pudo mostrar la comparación visual: {_e}")
    # 🔼 FIN INTEGRACIÓN 🔼
    ...
    try:
        ...
        if metricas1 and metricas2:
            ...
            print("⚠️  No se pudieron generar los gráficos (faltan métricas).")
    except Exception as _e:
        print(f"⚠️  No se pudo generar el gráfico comparativo: {_e}")
    # 🔼 FIN INTEGRACIÓN GRÁFICO 🔼
